...
'''
```
If you want to understand the time complexity of above function, it is O(n), since each element is iterated over till N.
...
'''
```
The time complexity for the above function is O(1), just one operation at constant time.
