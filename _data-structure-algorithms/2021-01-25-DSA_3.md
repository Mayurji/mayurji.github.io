...
Deque is also known as Double-Ended Queue. In normal queue, we perform insertion on rear side and deletion on front side of the queue following FIFO (First In First Out), but in **deque**, we can perform both insertion and deletion on both the sides of the queue.
...
Priority queue depends on how the priority is assigned. If priority is assigned to max values then we use **max-heap** and similar if priority smallest value then we create **min-heap**.
