...
In this blog post, we'll discuss and implement **Binary Tree** and its variants in python. **Binary Tree** is a tree where each parent node has **at most two child nodes**. There are different types of binary trees based on the number of leaf nodes or internal child nodes.

### Types of Binary Tree

<center>
<img src="{{site.url}}/assets/images/dsa/variant_BT.png" style="zoom: 5%; background-color:#DCDCDC;" width="80%" height=auto/><br>
<p>Figure 1: Variants of Binary Tree</p> 
</center>

**Full Binary Tree** - Each parent node or child node has either exactly two child nodes or no child node. \
**Perfect Binary Tree** - Each parent node or internal node has exactly two child nodes and the leaf nodes should be at the same level. \
**Complete Binary Tree** - It is similar to a full binary tree with few differences
   * All levels must be filled
   * All leaf nodes must lean towards left
   * The last leaf node may or may not have the right sibling.

**Degenerate or Pathological Tree** - A tree has a single child either on the left or right. \
**Skewed Binary Tree** - A tree dominated by either the left nodes or right nodes is a skewed tree. \
**Balanced Binary Tree** - The difference between the left and the right sub-tree for each node is either 0 or 1.

 Let's rewind a bit, we've discussed in a binary heap, how the position(index) of a node can be calculated from any node.

**Steps in finding an index of a node**
...
```python
        ...
        ### Every parent and the internal node should have at most two child nodes and the level should remain the same.
        ...
        def calculateDepth(node):
            ...
            #iterate through all the left of nodes from the root of the tree and count the depth!
...
```python
        ...
        def completeBinaryTree(root, index, total_nodes):
            ...
            # Check if the root node is empty, return true
            if root is None:
                return True
            
            # Checking the index of a node is greater than equal to total_nodes, if true then not a complete binary tree
...
Next, we'll continue with the binary search tree. If you've liked this post, please don't forget to subscribe to the newsletter.
