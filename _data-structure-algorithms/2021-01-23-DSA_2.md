...
---
In this blog post, we'll discuss three simple data structures: stack, queue, and heap & their implementation.

### Stack

Stack is a simple data structure and it stores elements like piles of plates. Operation in the stack is done on top of the stack meaning, we can perform insertion and deletion on the top pointer of the stack.

Consider a pile of 10 numbered plates with 1 at the bottom and 10 at the top, to remove plate 5, we need to remove 10, 9, 8, 7, 6 then 5 in that order, to perform the deletion. This procedure of deletion or accessing an element is called Last In First Out (LIFO). The things that come last, will be removed first.

Inserting an element in a stack is called **Push** and removing an element from the stack is called **Pop**.
...
Queue data structure follows the same operation as a queue in real life. Like a queue, while buying a ticket in a park, the person who buys the first ticket gets into the park first. It follows FIFO (First In First Out). Inserting into the queue is called **enqueue** and deletion of an element is called **dequeue**.
...
* Priority Queue - It's a special type of queue in which each element is associated with a priority and is served according to its priority. If elements with the same priority occur, they are served according to their order in the queue.
...
* Min Heap - the key of each node is always smaller than the child node/s and the key of the root node is the smallest among all other nodes.
...
It is the process of creating a heap data structure from a binary tree. It is used to create Max-Heap and Min-Heap.
...
4. Start from the first index of the non-leaf node whose index is given by *n/2 - 1*.  Since 6 elements, 6/2 -1 i.e. 2. Node at position 2 is selected.

5. Set current element * I* as *largest*

6. The index of a left child is given by *2i + 1* and the right child is given by *2i + 2*.
...
To add a new element into an existing heap, considering we have a max heap.

1. If a node is present in an existing heap with an empty child space, then we can add this new element as a child of that node, else we can create a new node.
...
To find the max or min of an element in the max or min-heap, find the root node of the binary tree.
...
- Heap is used for implementing **priority queue**.
- Dijkstra Algorithm
- Heap Sort
