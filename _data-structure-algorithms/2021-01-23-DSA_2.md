...
---
In this blog post, we'll discuss three simple data structures: stack, queue and heap & its implementation.

### Stack

Stack is a simple data structure and it stores elements like piles of plates. Operation in stack is done on top of stack meaning, we can perform insertion and deletion on top pointer of stack.

Consider a pile of 10 numbered plates with 1 at the bottom and 10 at the top, to remove plate 5, we need to remove 10, 9, 8, 7, 6 then 5 in that order, to perform deletion. This procedure of deletion or accessing an element is called Last In First Out (LIFO). The things that come last, will be removed first.

Inserting an element in stack is called **Push** and removing an element from stack is called **Pop**.
...
```
Time complexity of enqueue and dequeue operations in a queue using an array is `O(1)`.
