...
Note: the latest commit in this branch will be the same as the master branch because we created this branch while staying in the master branch. It is a version started from the master branch.
...
Now, the branch history will be merged i.e. the latest commit in master would be "Getting started with Menu Bar".
...
Now, Auto-Merging fails. Conflicted files show us both the changes we have made and ask us how to handle it. we can edit it manually and then add and commit again. We can go back to the files and keep the changes required and update the file.
...
Thus, we have learnt how to create branches, merge two branches and remove the conflict while merging. Next we'll see how to work with remote repositories.

### Working on Remote Repository

Let's clone the remote repository to the local system. Below command creates a folder with repo name with all the files from remote repository.
...
Note: if you clone a repo from local repo (git clone /git_basics), then it won't be remote. We need to add the cloned repo to the remote first.
...
The notification_bar branch should be available in **`git_basics`** repo as well. Changes made in the remote branch are available in the local repo.
