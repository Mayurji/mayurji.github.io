...
Consider a neural network with three layers (shallow network) which works good with 80% accuracy, and as I mentioned in the first point that stacking layers learns better features, I stack this network by appending three more layers which only acts as identity mapping, i.e., f(x) = x, f is my identity function, now instead of retaining the same performance, the network starts degrading. 

One way to look at this is that the network finds it difficult to map f(x)=x, and it would work better if f(x)=0, i.e., the shallow network. In simple terms, **Degradation problem can be referred to as the inability to resolve the optimization for the large network.**
