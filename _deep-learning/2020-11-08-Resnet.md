...
The next major issue with large neural networks is the **Degradation Problem.** It means as the number of layers increases the training accuracy starts saturating and then degrading. Consider a neural network with 3 layers (Shallow network) which works good with 80% accuracy and as I mentioned in the first point that stacking layers learns better features, I stack up by appending 3 more layers which only acts as identity mapping i.e. f(x) = x, f is my identity function, now instead of retaining the same performance, the network starts degrading. One way to look at this is that the network finds it difficult to f(x)=x and it would work better with f(x)=0 i.e. shallow network. In simple terms, **Degradation problem can be referred to as the inability to resolve the optimization for a large network.**

### **Resnet Breakthrough**
In 2015, the Microsoft research team came up with Resnet, **It found that splitting a deep network into three-layer chunks and passing the input into each chunk straight through to the next chunk, along with the residual-output of the chunk, helped eliminate much of this Vanishing Gradient Problem and Degradation Problem.** No extra parameters or changes to the learning algorithm were needed. ResNets yielded lower training error (and test error) than their shallower counterparts simply by reintroducing outputs from shallower layers in the network to compensate for the vanishing data. Note that **ResNets are an ensemble of relatively shallow nets and do not resolve the vanishing gradient problem by preserving gradient flow throughout the entire depth of the network – rather, they avoid the problem simply by constructing ensembles of many short networks together.**
...
Resnet Block's output is H(x), which is equal to F(x) + x. Assuming our objective function of Resnet Block is H(x). The author's Hypothesize is that learning a function F(x) is simpler than H(x), H(x) a combination of input and output from a two-layered network. H(x) depends on F(x), which is the output of a two-layer network. F(x) is called Residual Mapping and x is called Identity Mapping / Skip Connections.
 ...
 **x → weight1 → ReLU → weight2 → ReLU... → x**
 ...
 So, approximating the identity mapping with all these weights and ReLUs in the middle would be difficult. Now, if we define the desired mapping H(x)=F(x)+x, then we just need to get F(x)=0 as follows.
...
Identity Mapping is also known as Skip Connections, has been experimented with widely by different elements like Dropout, Conv1x1 or multiplying with a constant factor, etc. Check out the identity mapping paper.\
These experiments of skip connections show varied results i.e. find in the second image below.\
...
Since we are adding F(x) + x, the dimensions should match. If dimensions don't match, we can follow the below techniques to match the dimension.
 > Either we can pad zeros to dimension to match F(x) with x or viceversa\
   We can use the conv1x1 layer, reduce or increase the dimension. It introduces parameters and which in turn increases the time complexity.
...
The purpose of using Conv1x1 is to make the dimension match between mapping function F(x) and x or h(x) as mentioned in the identity mapping paper. As given in referenced links, Conv1x1 helps to reduce or increase the number of feature maps i.e. which helps in dimension matching between resnet mapping and identity mapping.

1x1 is the kernel size of the convolutional operation. Conv1x1 is a projection shortcut i.e. it comes into the picture when F(x) and x dimensions differ. Otherwise, Identity Mapping is done without Conv1x1. Conv1x1 increases the time complexity, because of parameters to learn.
...
The below image is a sequential module with resnet block. It is layer 2 of the resnet network. In Layer 2, resnet block's input channel(feature map) is 64 and the output channel is 128. Since the Output channel is changed to 128 we need to change our identity mapping to meet the dimension of this output channel. After Residual Mapping is done as mentioned in paper F(x), we bring in conv1x1 through the downsampling layer.
