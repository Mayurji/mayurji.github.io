...
**Bias** in statistical term refers to the tendency of a measurement process to over- or under-estimate the value of a population parameter. In survey sampling, for example, bias would be the tendency of a sample statistic to systematically over- or under-estimate a population parameter. In machine learning, a model is said to be suffering from bias when the model does not perform well on training set i.e. the model is unable to recognize the data pattern of training set, making it too simplistic. As the model complexity is increased, the bias is reduced.
