...
Data is at the heart of machine learning. It is believed that the larger the corpus of data, the better is the generalization of the learning model. Less data results in poor generalization, thus causing the model to perform poorly on unseen data.
...
A model is a learnable function, which learns based on data. Complex data with hundreds of features requires complex function. 

For instance, appraisals in the workplace is a complex function because various factors play a key role in your appraisal like your effort in a team, quality deliverables, years of experience, cordial relation with managers 😉 etc. We cannot simply make a linear function like increasing salary with increasing years of experience to model such problem. 

Hence, the model learns these different factors and its interrelation and then extracts the hidden patterns from the data, these patterns are generalized over a function to make up a model.

**Learning**

Learning, a model is said to learn from data. Learning here refers to identifying or recognizing the pattern in the data. 

A model performs well, if it predicts accurately on the unseen data. Unseen data is technically called test data. There are few terms related to learning like generalization, test error, validation error which we will discuss in the part of the series.
...
**Predictor**, are a set of variables, which helps in describing the system. From above patient records example, if we want to predict, if the patient has disease XYZ, then our predictor variable will include health habits (like smoking, drinker etc), existing health condition like blood pressure, diabeitc etc. Predictor is also referred as independent variable.
...
Labels are also called target or dependent variable. Dependent as it depends on predictor variables.

**Model** is a machine learning algorithm, which is applied or trained on data like patients records to predict if a patient has disease XYZ. 
...
These tunable parameters are **Hyperparameter**, which are set manually by a user who is performing machine learning. Other set of parameters are learnt by machine learning algorithm like *coefficient of X's* in Linear regression, Weights in Deep Learning Algorithms etc.

**Dataset** is the data on which we apply machine learning algorithm after cleaning and structuring it in the form, such that it is consumable by ml algorithm. Consider we have 1000 patient records with target as *disease XYZ* with value 0 or 1. 0 represents no disease and 1 represents disease. To perform machine learning, we split the dataset into multiple sets like train set, validation set and test set. 
...
**Training** is a process to learn the parameters of the machine learning algorithm. During this process, we keep track of training error, as the training continues to take place, the training error reduces. Training is done on a train set.

**Evaluating** is a process to learn hyperparameters and try out or experiment with the best possible hyperparameter for the learning algorithm. Evaluation can take place simultaneously with training. During evaluation, we keep track of validation errors. Evaluation is done on the validation set of the dataset.
...
A dataset has features ( X's ) and target ( y ). The target can be a discrete value or a continuous value based on which we can call the problem as Classification or Regression respectively.

*Classification*

The problem is termed as either Binary or Multiclass classification. The target variable for classification problem is either 0/1 or else multiple values like Dogs, Person, Cat etc. It has **categorical outcomes**.
...
Each dataset has features (X) and target (y) variables. These features are attributes which affect the target variable either directly or indirectly. Now, this relationship between X and y is built by a machine learning algorithm.
...
**Unsupervised Learning**
   ...
   The Algorithm learns from unlabeled data. A dataset with no label is trained using unsupervised learning i.e. the patterns of such a dataset is learned by using clustering techniques or other self-supervised approach.
...
As part of this series, I will share separate blogs on each topic. Please make sure to subscribe the newsletter if you wish to get the update on future articles. Thanks for reading.