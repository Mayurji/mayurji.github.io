...
The idea is to rebalance the class distribution by resampling the data space. Resampling is done by either adding more minority samples or removing majority samples. It avoids the modification of the learning algorithm by trying to decrease the effect caused by data imbalance with a preprocessing step, so it is usually more versatile than the other imbalance learning methods. The algorithms learning about imbalance data is called **Imbalance Learning**.
...
In this resampling technique, the minority samples are replicated randomly to create a balanced class distribution. This method might cause overfitting to minority samples since it makes exact copies of existing samples.
...
SMOTE (Synthetic Minority Over-sampling Technique) is a popular oversampling technique. The main idea is to create a new instance of minority samples by interpolating several minority class instances that lie together. SMOTE uses k-nearest neighbors to create synthetic examples of the minority class. SMOTE can avoid the overfitting problem. A major disadvantage of SMOTE is that While generating synthetic examples, SMOTE does not take into consideration neighboring examples can be from other classes. This can increase the overlapping of classes (making wrong labeling) and can introduce additional noise.

To overcome SMOTE issues, there is a modified SMOTE called MSMOTE. The main idea of this algorithm is to divide the instances of the minority class into three groups, **safe, border, and latent noise instances**, by the calculation of distances among all samples. We create boundaries for each sample in the minority class to create the new instance of minority class thus helping in reducing the mislabelled samples. 

#### Undersampling

In this resampling technique, the majority of class samples are randomly dropped to create a balanced class distribution. The downside is we might lose some potentially important information about the majority class.
...
**Biasing toward rare classes**: As mentioned above, the model assigns more weights to the majority class, it can also be termed as that, it'll bias toward majority class and making the wrong prediction on minority class. What if we punish the model for making wrong predictions on minority classes to correct this bias?
...
A simple weighted loss function can be written as follows:

The weight of the class is inversely proportional to the number of samples in that class, which makes the class with fewer samples have more weights.
<p>
$$
...
A more sophisticated version of this loss can take into account the overlapping among existing samples, such as [Class-Balanced Loss Based on Effective Number of Samples](https://openaccess.thecvf.com/content_CVPR_2019/papers/Cui_Class-Balanced_Loss_Based_on_Effective_Number_of_Samples_CVPR_2019_paper.pdf) (Cui et al., CVPR 2019).

**Biasing toward difficult samples**: When a model is trained, the model will predict with great confidence on certain samples and may perform poorly or predict with less confidence on certain samples. The idea here is to make the model be incentivized towards samples that are difficult to learn. What if, we can adjust the weights too high, whenever a sample has a lower probability of being right. Focal Loss is a loss function, which does exactly what we have described before.
...
We'll discuss two ensemble models bagging and boosting for class imbalance. First, we'll understand what these models are and how they can help in class imbalance.

#### Bagging :

How does it work? Instead of training a model on the entire dataset at one time, we create multiple subsamples of the dataset with replacement, making different datasets, called bootstraps. And then we train a model on each of these bootstraps. Sampling with replacement ensures each bootstrap is independent of its peers.
